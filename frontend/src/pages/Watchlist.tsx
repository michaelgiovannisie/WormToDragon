...
export default function Watchlist() {
  ...
  const [active, setActive]             = useState<any>(null);   // selected watchlist with items
  ...
  const loadActive = (id: string) =>
    fetch(`${API}/watchlists/${id}`)
      .then(r => r.json()).then(d => { setActive(d); }).catch(console.error);

  useEffect(() => { loadLists(); }, []);

  // Local asset search (library only)
  useEffect(() => {
  ...
  return (
    <div style={{ color: C.text, fontFamily: C.font }}>
      ...
      <h2 style={{ fontSize: "48px", marginTop: "12px", marginBottom: "4px" }}>Watchlists</h2>
      <p style={{ color: C.muted, marginBottom: "40px" }}>Track assets you're monitoring without holding them.</p>
